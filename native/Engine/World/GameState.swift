...
final class GameState {
    ...
    private func startMinigame(importPath: String, mapName: String?) {
        ...
        case .tennis3d:
            ...
            delegate?.gameStateDidStartMinigame(game)
        case .schoolrush:
            let game = SchoolRushGame(host: self, npcs: npcs, myCharacter: player.appearance)
            minigame = game
            game.start()
            delegate?.gameStateDidStartMinigame(game)
        case .football:
            ...
            delegate?.gameStateDidStartMinigame(game)
        case .fivenights:
            let game = FiveNightsGame(host: self, npcs: npcs, myCharacter: player.appearance)
            minigame = game
            game.start()
            delegate?.gameStateDidStartMinigame(game)
        case .schoolescape:
            let game = SchoolEscapeGame(host: self, npcs: npcs, myCharacter: player.appearance)
            minigame = game
            game.start()
            delegate?.gameStateDidStartMinigame(game)
        }

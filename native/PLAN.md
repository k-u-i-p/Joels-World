...
**Nothing is fetched over HTTP. Both apps ship everything they need.** `assets/` and `data/`
sit at the repository root; `tools/assets/stage.sh` runs from a build phase in both targets and
copies the shipped subset into `GameAssets/` inside the app. `AssetLocator` resolves the paths
the world data is written in — `/media/laser.mp3`, `/junior_school/chunks/background_3_2.jpg` —
against that directory.

`assets/` is a *working* tree, which is why the ship-list is explicit rather than "copy
everything": the full-size map layers the slicer reads are ~90 MB and are input, not output.

| Shipped | Size | Notes |
|---|---|---|
| `models/` GLB (props, heads, shoes, torso) | 104 MB | Needed at spawn |
| `media/` audio | 11 MB | Emotes, ambience, minigame sound |
| `<map>/chunks/` map tiles | 37 MB | Generated by `slice_maps.js`, not committed |
| `avatars/`, `minimaps/` | ~1 MB | |
| `<map>/clip_mask.*`, `<map>/*.glb` | ~1 MB | Walkability masks and map props |
| `minigames/tennis/map.svg` | 2.3 MB | Rasterised on device; iOS has no SVG decoder |
| `data/` | 76 KB | `maps.json`, and each map's `objects.json` and `npc.json` |
| **Total** | **155 MB** | Against a 247 MB working tree |

Not shipped: the full-size map layers, and `fonts/` and `icons/`, which were the web build's.

`Resources/emoji/` — 23 Twemoji SVGs, 104 KB, **CC-BY 4.0** — is separate, a fallback used
where the platform cannot draw a colour emoji, which today means the simulator. See
`Resources/emoji/ATTRIBUTION.txt`.

### Why the tiles are bundled too

The earlier plan kept map tiles server-fetched, on the grounds that bundling them "would bloat
the app for tiles most players never see". That was wrong about the proportions: of the 264 MB
the server used to host, the tiles are 37 MB and the models are 104 MB. Bundling the tiles
costs a seventh of the app to remove the last reason for the server to serve anything.
...
**`init` carries the live half of a connection only** — `mapId`, `characters`, `myCharacter`,
around 650 bytes. The map definition, the map list, the objects and the NPCs used to ride along
with it; `WorldData` reads them out of the bundled `data/` instead, selected by `mapId`.

`objects_update` and `npcs_update` survive, and matter more than they did: the server watches
`data/**/objects.json` and `npc.json`, so when the macOS editor writes one, every connected
client gets the change. That is the whole of "the server only sends updates".
...
| **7b. Tag** | Tag (591 lines) | **deleted** — deferred on 2026-08-08 as "rubbish and needs reworking", then removed outright the same day: the map, its art, its `MinigameKind` case and the NPC who was its door |
| **9. macOS admin app** | `admin.js` ported to a native Mac editor on the shared engine | **done** — see `PROGRESS.md` |
| **8. Retire web** | Delete `client/`, move the asset tree under `server/`, strip static hosting | **done** |
| **10. Strip the server** | Bundle the assets and the world into the apps; reduce the server to `node:http` + `ws` | **done** — see §5, §6 and §9 |
...
*Paths above are as they stood on 2026-08-08.* Phase 10 moved the tree again and stopped
serving it: `server/assets` → `assets/`, the three scripts → `tools/assets/`, and `static.js`
was deleted rather than repointed. See §9.

## 9. Phase 10: the server stops serving

The server was a WebSocket relay wearing an Express application. Three jobs went:

| What | Why it went |
|---|---|
| `cookieParser` + the HTTP session middleware | No browser exists. The socket handshake does its own token lookup, and nothing read `req.session`. Worse, it minted a session file for every cookie-less HTTP request — that is, every asset fetch. 269 files of `{}` had accumulated |
| `/api/config` | Served the emote *names*, built by regexing a file. The app compiles in the definitions; `EmoteCatalog` reads `Emotes.table` |
| The static asset mount | 155 MB of it ships in the app instead (§5) |
...
**Sessions are untouched.** They are how play state resumes; only the HTTP-side cookie path
went.

### The editor writes files now

`server/admin.js` and the fourteen socket messages that fed it are gone. `WorldFileStore`
applies the same fourteen operations to `data/` on disk, and the server's existing `fs.watch`
on those files turns a save into an `objects_update` broadcast for everyone connected.

The one thing this cost is a serialiser. Those files are authored content under version
control, and Node wrote them with `JSON.stringify(value, replacer, 2)`; `JSONSerialization`
would have reordered every key in the file and turned a two-line change into a whole-file
diff. `OrderedJSON` is a JSON model that keeps object key order, plus a parser and a writer
that matches `JSON.stringify` byte for byte — including printing a whole number as `40` rather
than `40.0`. The editor's `-selftest` asserts it: it baselines both files, drives a create →
event-tree write → delete round trip and three drag-and-restore cycles, and checks the bytes
come back identical.

...
function processNPCLogic(npc, itPlayer, dt) {
  ...
  if (isIt) {
    ...
    if (itPlayer) {
      ...
      // If IT is far away, wander randomly instead of sticking to standard flee
      const dist = Math.sqrt((itPlayer.x - npc.x) ** 2 + (itPlayer.y - npc.y) ** 2);
      if (dist > 400) {
        // Pseudo random wander using time
        dx = Math.cos(Date.now() / 1000 + npc.id) * 100;
        dy = Math.sin(Date.now() / 1500 + npc.id) * 100;
      }
    }
  }
  ...
  if (moveX !== 0 || moveY !== 0) {
    ...
    npc.rotation = lerpRotation(npc.rotation, targetAngle, 600 * dt);
    npc.legTimer += 10 * dt;
  }
}
